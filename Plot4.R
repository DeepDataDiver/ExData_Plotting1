...
#------------------------------------------------------------------------
# Task 2.4: Create Plot4 - Multiple base plots
#------------------------------------------------------------------------
...
# plot in cell 1 
with(DT, plot(strptime(z, "%d/%m/%Y %H:%M:%S"), as.numeric(Global_active_power), type="l", xlab="", ylab="Global Active Power"))
# plot in cell 2 
with(DT, plot(strptime(z, "%d/%m/%Y %H:%M:%S"), as.numeric(Voltage), type="l", xlab="datetime", ylab="Voltage"))
# plot in cell 3
...
legend("topright", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=c(1,1), col=c("black", "red", "blue"), bty="n", pt.cex=0.35)
# plot in cell 4
with(DT, plot(strptime(z, "%d/%m/%Y %H:%M:%S"), as.numeric(Global_reactive_power), type="l", xlab="datetime", ylab="Global_reactive_power"))


# open png device and points it to destination file 
png(file="./figure/plot4.png", width=480, height=480)

# paint multiple plots
# set a 2 x 2 grid 
par(mfrow=c(2,2), mar=c(5, 5, 2, 2))
